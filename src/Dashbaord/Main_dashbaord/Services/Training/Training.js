...
import {
  ...
  cancel_appointment
} from "../../../../controllers/services/trainingController";
 ...
 *********************************************************/
  ...
  const getBookingList = async () => {
    try {
      ...
      console.log("training bookingDetails", listData)
  ...
  const handleViewDetails = (bookingId, isPickUp) => {
    ...
    navigate("/user/training-order", { state: { bookingDetails: bookingId, isPickUp } });
  };
  ...
  return (
    <>
      ...
      {List.map((booking, index) => (
        <div className="grooming_listing" key={booking._id} onClick={() => handleViewDetails(booking, booking?.pick_drop)}>
