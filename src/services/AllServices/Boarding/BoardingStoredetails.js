...
import { details as boarding_services } from "../../../controllers/services/boardingController";
...
import  OrderNow  from "./BookPopUp";

const BoardingStoredetails = () => {
    const token = sessionStorage.getItem("TOKEN");
    const [loginPopupShow, setLoginPopupShow] = useState(false);
    const togglelLoginPopupShow = () => {
        // console.log('popup',!loginPopupShow);
   ...
   *********************************************************/
    const getDetails = async () => {
        ...
        try {
            ...
            const boardingIds = storedData.data.find((item) => item.slug === "boarding");
            setCategortId(boardingIds?._id);
            ...
            if (listData?.status === true) {
                ...
                if (listData.result?.storeData?._id && listData.result?._id) {
                    ...
                    const boardingData = await boarding_services(boardingOptions);
                    console.log("boardingDataaaa", boardingData)
                    if (boardingData?.status === true) {
                        setStoreDetails(prevDetails => ({
                            ...prevDetails,
                            boardingServices: boardingData.result
                        }));
                    } else {
                        console.log("Failed to fetch boarding services");
                    }
                }
            }
        } catch (error) {
    ...
    *********************************************************/
    ...
    return (
        <>
            ...
            {/* Boarding service section */}
            <div className="">
                <div className="container">
                    <div className="row">
                        <div className="col-md-3">
                            <InnerServiceSidebar slug="boarding" vendorData={StoreDetails?.vendorData} storeId={StoreDetails?._id} StoreDetails = {StoreDetails} category ={categoryId}/>
                        </div>
                        <div className="col-md-9 grooming_info">
                            <BookedServiceslider />
                            <div className="features_grooming ">
                                <h1 className="main_heading" ><span>Boarding </span> Service</h1>
                                <p className="sub_heading">It is a long established fact that a reader will be distracted by the readable content</p>
                            </div>

                            {StoreDetails?.boardingServices?.length > 0 ? (
                                StoreDetails.boardingServices.map((item, index) => (
            ...
            {popUp && <OrderNow closePopUp={closePopUp} 
...
export default BoardingStoredetails;