...
function Checkout_detail() {
  ...
  // const treat = (bookingData?.servicesId?.price?.toFixed(2)) / 10;
  ...
  const handleCreateBooking = async () => {
    ...
    const services = [
      {
        ...
        total: bookingData?.servicesId?.price,
      },
    ];
    ...
    formData.append("appointment_date", bookingData?.selectedDate);
    ...
    // pickupInfo?.pickupDropAddress?.address
      ...
      const paymentUrl = response.res.data.transaction.url; // URL for the payment gateway
        const chargeId = response.res.data.id;
              ...
              if (paymentUrl) {
                  ...
                  sessionStorage.setItem("daycaretransaction", JSON.stringify(transaction));
                  sessionStorage.setItem("daycarechargeId", JSON.stringify(chargeId));
                  const treats = treat;
                  sessionStorage.setItem("daycareresult", JSON.stringify(result));
                  sessionStorage.setItem("daycaretreats", JSON.stringify(treats));
              }
  ...
  // },[]);
  useEffect(() => {
    if (bookingData?.servicesId?.price !== undefined && pickupPrice?.total_charges !== undefined) {
  ...
  }, [bookingData?.servicesId?.price, pickupPrice, treatsUsed, discountAmount]);  // Add all relevant dependencies here
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td className="text-end">
                        AED {bookingData?.servicesId?.price?.toFixed(2)}
                      </td>
                    </tr>
                    <tr>
                      <td>Tier-Discount</td>
