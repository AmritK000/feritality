...
import { booking as daycare_services_appointment } from "../../../controllers/services/dayCareController.js";
...
const BookNew = ({
  ...
  const getAppointment = async () => {
    try {
      ...
      const appointmentTime = await daycare_services_appointment(options);
  ...
  const handleBookNow = () => {
    ...
    const enBookingData = btoa(JSON.stringify(bookingData));
    const oneHourFromNow = new Date(new Date().getTime() + 60 * 60 * 1000);
    // Store booking data in a cookie (expires in 1 day)
    ...
    if (storedCookie) {
      ...
      navigate("/services/day-care/pick-drop");
    } else {
