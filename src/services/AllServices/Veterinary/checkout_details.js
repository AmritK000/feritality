...
import { create as OrderPlaced, changeStatus } from "../../../controllers/services/veterinaryController"
...
function Checkoutdetail() {
   ...
   *********************************************************/
    ...
    const total = checkoutData?.checkoutData?.servicesId?.price - discountAmount;
    ...
    const handleSubmit = async (e) => {
        ...
        form.append('owner_address', checkoutData?.selectedAddress?.address ? checkoutData?.selectedAddress?.address : userInfo.address);
          ...
          const paymentUrl = response.res.data.transaction.url; // URL for the payment gateway
            const chargeId = response.res.data.id;
                  ...
                  if (paymentUrl) {
                      ...
                      sessionStorage.setItem("veterinarytransaction", JSON.stringify(transaction));
                      sessionStorage.setItem("veterinarychargeId", JSON.stringify(chargeId));
                      const treats = treat;
                      sessionStorage.setItem("veterinaryresult", JSON.stringify(result));
                      sessionStorage.setItem("veterinarytreats", JSON.stringify(treats));
                  }
