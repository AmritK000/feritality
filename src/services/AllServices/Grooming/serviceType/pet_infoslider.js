...
const Pet_infoslider = ({petData, setPetData, setshow_petdetail}) => {
    ...
    try {
      ...
      if (listData?.status === true) {
        ...
        if (listData.result.length > 0) {
          setshow_petdetail(false); // Don't show the form if pets are available
        } else {
          setshow_petdetail(true); // Show the form if no pets are available
        }
      }
      setIsLoading(false);
    } catch (error) {
