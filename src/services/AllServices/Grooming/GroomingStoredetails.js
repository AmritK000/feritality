...
import { details as grooming_services } from "../../../controllers/services/groomingController";
...
import OrderNow from "../../../components/Modals/Booknew"
import { ListEffectSkelton } from "../../../components/SkeltonEffect/ListEffectSkelton";

const GroomingStoredetails = () => {
    ...
    /*********************************************************
   *  This function is use to fetch grooming list
   *********************************************************/
        ...
        try {
            ...
            if (listData?.status === true) {
                ...
                if (listData.result?.storeData?._id && listData.result?._id) {
                    ...
                    const groomingData = await grooming_services(groomingOptions);
                    if (groomingData?.status === true) {
                        setStoreDetails(prevDetails => ({
                            ...prevDetails,
                            groomingServices: groomingData.result
                        }));
                    } else {
                ...
                <div className="container">
                    <div className="row">
                        <div className="col-md-3">
                            <InnerServiceSidebar slug="grooming" vendorData={StoreDetails?.vendorData} storeId={StoreDetails?._id} StoreDetails = {StoreDetails} category ={categoryId}/>
                        </div>
                        <div className="col-md-9 grooming_info">
                            <BookedServiceslider />
                            <div className="features_grooming ">
                                <h1 className="main_heading" ><span>Grooming</span> Services</h1>
                                {/* <p className="sub_heading">It is a long established fact that a reader will be distracted by the readable content</p> */}
                            </div>
                            {StoreDetails?.groomingServices?.length > 0 ? (
                                StoreDetails.groomingServices.map((item, index) => (
                                        ...
                                        <Link to={false}>
                                            ...
                                            <div className="rightside_list">
                                                ...
                                                <div className="connect_btn">
                                                    {item?.store?.center_facilities?.in_store === "Y" && (
...
export default GroomingStoredetails;